...
function App() {
    ...
    useEffect(() => {
        ...
        if(!loadingChannel){
            ...
            loadAllChannels(agregationsMultipliers[aggregation], channelsNumber)
        }
    ...
    function loadAllChannels(agregation, channelsNumber) {
        for(let channel=0;channel<channelsNumber;channel++) {
            dispatch(fetchChannel(dataForCharts, channel, agregation))
        }
